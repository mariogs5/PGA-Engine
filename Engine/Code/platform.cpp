...
void OnGlfwKeyboardEvent(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    switch (key) {
        ...
        case GLFW_KEY_Y: key = K_Y; break; case GLFW_KEY_Z: key = K_Z; 
        
        break; case GLFW_KEY_LEFT_SHIFT: key = K_LEFT_SHIFT; break;
        break; case GLFW_KEY_LEFT_ALT: key = K_LEFT_ALT; break;
...
void OnGlfwResizeFramebuffer(GLFWwindow* window, int width, int height)
{
    ...
    app->OnResizeWindow(width, height);
}
